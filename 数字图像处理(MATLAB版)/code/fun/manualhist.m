...
%   of the histogram. The number of elements in the histogram vector P
%   is 256 and sum(P) is normalized to 1. MANUALHIST repeatedly 
%   prompts for the parameters and plots the resulting histgram until
%   the user types an 'x' to quit, and then it returns the last
%   histgram computed.
%
...
while repeats
    s = input('Enter ml, sig1, m2, sig2, A1, A2, k OR x to quit:',...
        's');
    if strcmp(s, quitnow)
        break
    end
    % Convert the input string to a vector of numerical valuss and verify
